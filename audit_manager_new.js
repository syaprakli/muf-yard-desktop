...
const AuditManager = {
    ...
    promptNewAudit: () => {
        ...
        <div id="${modalId}" class="modal" style="display:flex; justify-content:center; align-items:center; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:10000;">
            <div class="modal-content" style="background:var(--bg-card); padding:2rem; border-radius:1rem; width:400px; box-shadow:0 10px 25px rgba(0,0,0,0.2);">
                <h3 style="margin-bottom:1rem; color:var(--text-main);">Yeni Denetim Başlat</h3>
                <div class="form-group" style="margin-bottom:1.5rem;">
                    <label style="display:block; margin-bottom:0.5rem; color:var(--text-secondary); font-size:0.9rem;">Denetim İsmi</label>
        ...
        const create = () => {
            ...
            if (name) {
                AuditManager.createNewAudit(name);
    ...
    createNewAudit: (name) => {
        ...
        const newRecord = {
            ...
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString(),
            data: { form: {}, info: {}, notes: '', photos: [] }, // New Data Structure
            progress: 0
        };
    ...
    handlePhotoUpload: async (input) => {
        if (input.files && input.files[0]) {
            try {
                ...
                const Toast = window.Toast || { show: console.log }; // Fallback
    ...
    exportToExcel: async () => {
        try {
            ...
            if (typeof require !== 'undefined') {
                ...
                if (!fs.existsSync(templatePath)) templatePath = path.join(__dirname, 'Sablonlar', 'denetim_formu.xlsx');
                if (!fs.existsSync(templatePath) && process.resourcesPath) templatePath = path.join(process.resourcesPath, 'Sablonlar', 'denetim_formu.xlsx');
        ...
        setTimeout(async () => {
            try {
                ...
                if (typeof require !== 'undefined') {
                    ...
                    if (!fs.existsSync(filePath)) filePath = path.join(__dirname, 'Sablonlar', 'denetim_formu.xlsx');
                    if (!fs.existsSync(filePath) && process.resourcesPath) filePath = path.join(process.resourcesPath, 'Sablonlar', 'denetim_formu.xlsx');
